...
#define tiger1 (tiger_table)
#define tiger2 (tiger_table+256)
#define tiger3 (tiger_table+256*2)
#define tiger4 (tiger_table+256*3)

static const ulong64 tiger_table[4*256] = {
...
class CTiger : public CHashUpdate<CTiger>
{
	TIGER_CTX m_context;

	void tiger_round(ulong64 *a, ulong64 *b, ulong64 *c, ulong64 x, int mul)
	{
		ulong64 tmp;
		tmp = (*c ^= x); 
		*a -= tiger1[byte(tmp, 0)] ^ tiger2[byte(tmp, 2)] ^ tiger3[byte(tmp, 4)] ^ tiger4[byte(tmp, 6)];      
		tmp = (*b += tiger4[byte(tmp, 1)] ^ tiger3[byte(tmp, 3)] ^ tiger2[byte(tmp,5)] ^ tiger1[byte(tmp,7)]); 
		switch (mul) 
		{
	...
	/* one complete pass */
	void pass(ulong64 *a, ulong64 *b, ulong64 *c, ulong64 *x, int mul)
	{
...
public:
	...
	int Compress(unsigned char *buf)
	{
		...
		/* store state */
		m_context.state[0] = a ^ m_context.state[0];
		m_context.state[1] = b - m_context.state[1];
		m_context.state[2] = c + m_context.state[2];
	...
	int Init()
	{
		m_context.state[0] = CONST64(0x0123456789ABCDEF);
		m_context.state[1] = CONST64(0xFEDCBA9876543210);
		m_context.state[2] = CONST64(0xF096A5B4C3B2E187);
		m_context.curlen = 0;
		m_context.length = 0;
		return 0;
	}
	...
	{
		return CHashUpdate<CTiger>::Update(pData, 
			ulLen, 
			m_context.curlen, 
			m_context.length, 
			m_context.buf, 
	...
	int Final(unsigned char *pszOut)
	{
		...
		/* append the '1' bit */
		m_context.buf[m_context.curlen++] = (unsigned char)0x01;
		...
		* encoding like normal. */
		...
		/* copy output */
		STORE64L(m_context.state[0], &pszOut[0]);
		STORE64L(m_context.state[1], &pszOut[8]);
		STORE64L(m_context.state[2], &pszOut[16]);
