#ifndef HASH_WHIRLPOOL_H_
#define HASH_WHIRLPOOL_H_
...
#define WHIRLPOOLGB(a,i,j) ((a[(i) & 7] >> (8 * (j))) & 255)

/* shortcut macro to perform three functions at once */
#define theta_pi_gamma(a, i)             \
	SB0(WHIRLPOOLGB(a, i-0, 7)) ^                 \
	SB1(WHIRLPOOLGB(a, i-1, 6)) ^                 \
	SB2(WHIRLPOOLGB(a, i-2, 5)) ^                 \
	SB3(WHIRLPOOLGB(a, i-3, 4)) ^                 \
	SB4(WHIRLPOOLGB(a, i-4, 3)) ^                 \
	SB5(WHIRLPOOLGB(a, i-5, 2)) ^                 \
	SB6(WHIRLPOOLGB(a, i-6, 1)) ^                 \
	SB7(WHIRLPOOLGB(a, i-7, 0))
...
class CWhirlPool : public CHashUpdate<CWhirlPool>
{
	WHIRLPOOL_CTX m_context;
public:
	...
	{
		return CHashUpdate<CWhirlPool>::Update(pData, 
			ulLen, 
			m_context.curlen, 
			m_context.length, 
			m_context.buf, 
